...
public class DrumClassifier : MonoBehaviour
{
    [Header ( "Prediction Result UI" )]
    public TextMeshProUGUI resultText; //예측 결과 텍스트
    public TextMeshProUGUI failCountText; //실패 구간 텍스트
    public TextMeshProUGUI successCountText; //성공 구간 텍스트
    ...
    [Header ( "AI" )]
    public ModelAsset drumPredictionModel;  // ONNX 모델 (Unity Sentis)
    Worker worker;
    Tensor<float> tensor;
    private const int windowSize = 10;  // LSTM 입력 시퀀스 길이 (10 프레임)
    private const int targetLength = 2760; // 학습에 사용한 보간 데이터 길이
    ...
    void PredictSuccess ( )
    {
        ...
        for ( int startIdx = 0 ; startIdx < numPredictions * stepSize ; startIdx += stepSize )
        {
            ...
            {
                for ( int j = 1 ; j < 7 ; j++ ) // 타임스탬프 제외하고 예측
                {
            ...
            // 모델 예측
            using ( var tensor = new Tensor<float> ( new TensorShape ( 1 , windowSize , 6 ) , inputArray ) )
            {
                worker. Schedule ( tensor );

                // 출력 텐서 처리
                using ( var outputTensor = worker. PeekOutput ( "dense_3" ) as Tensor<float> )
                {
    ...
    void ProcessPredictions ( List<float> predictions )
    {
        ...
        if ( averagePrediction > 0.5f )
        {
            ...
            resultText. text = "Drum Failure";
            failCountText. text = ( $"Fail Prediction Count: {failcount}" );
            successCountText. text = ( $"Success Prediction Count:{successCount}" );
