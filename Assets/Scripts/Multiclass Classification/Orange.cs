...
public class Orange : MonoBehaviour
{
    ...
    Worker worker;
    Tensor<float> tensor;
    private const int windowSize = 50;  // LSTM 입력 시퀀스 길이 (10 프레임)
    private const int targetLength = 251; // 학습에 사용한 보간 데이터 길이
    private const int numFeatures = 7;

    private readonly float [ ] means = new float [ numFeatures ] { 0.95840532f, -0.01980872f , 0.78997128f , -0.53599932f , -0.87505889f , -0.42054741f , 0.00134831f , };
    private readonly float [ ] stdDevs = new float [ numFeatures ] { 0.56429057f , 0.09325995f , 0.15870447f , 0.23577143f , 10.29126424f , 12.45190583f , 8.71138474f , };
    ...
    void ProcessPredictions ( List<int> predictions )
    {
        ...
        switch ( majorityClass )
        {
            ...
            case 1:
                ...
                resultText. text = $"실패 1: 손의 위치가 위로 올라가면서 정상 범위를 벗어났습니다.";
                break;
            case 2:
                Debug. Log ( "실패 2: 손의 위치가 아래로 내려가면서 정상 범위를 벗어났습니다." );
                resultText. text = $"실패 2: 손의 위치가 아래로 내려가면서 정상 범위를 벗어났습니다.";
                break;
            case 3:
                Debug. Log ( "실패 3: 손의 위치가 오른쪽으로 움직이면서 정상 범위를 벗어났습니다." );
                resultText. text = $"실패 3: 손의 위치가 오른쪽으로 움직이면서 정상 범위를 벗어났습니다.";
                break;
            case 4:
                Debug. Log ( "실패 4: 손의 위치가 왼쪽으로 움직이면서 정상 범위를 벗어났습니다." );
                resultText. text = $"실패 4: 손의 위치가 왼쪽으로 움직이면서 정상 범위를 벗어났습니다.";
                break;
            default:
