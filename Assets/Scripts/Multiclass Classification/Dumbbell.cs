...
public class Dumbbell : MonoBehaviour
{
    ...
    private const int targetLength = 234; // 학습에 사용한 보간 데이터 길이
    private const int numFeatures = 7;

    private readonly float [ ] means = new float [ 7 ] { 0.88125613f,  -0.20148076f , -0.39596195f , 0.36610439f , -73.63991595f , 14.42237686f , 11.09106747f , };
    private readonly float [ ] stdDevs = new float [ 7 ] { 0.54643425f , 0.29020117f , 0.38497557f , 0.69811753f , 64.0749609f , 29.45186231f , 45.86507804f , };
    ...
    void ProcessPredictions ( List<int> predictions )
    {
        ...
        switch ( majorityClass )
        {
            ...
            case 1:
                ...
                resultText. text = $"실패 1: 팔꿈치와 팔목 사이의 각도가 정상 범위보다 부족합니다.";
                break;
            case 2:
                Debug. Log ( "실패 2: 팔의 위치가 왼쪽으로 이동되어 정상 범위를 벗어났습니다." );
                resultText. text = $"실패 2: 팔의 위치가 왼쪽으로 이동되어 정상 범위를 벗어났습니다";
                break;
            case 3:
                Debug. Log ( "실패 2: 팔의 위치가 왼쪽으로 이동되어 정상 범위를 벗어났습니다." );
                resultText. text = $"실패 2: 팔의 위치가 오른쪽으로 이동되어 정상 범위를 벗어났습니다";
                break;
            default:
