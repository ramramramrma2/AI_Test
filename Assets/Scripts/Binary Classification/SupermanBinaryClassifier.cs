...
public class SupermanBinaryClassifier : MonoBehaviour
{
    ...
    [Header ( "AI" )]
    public ModelAsset supermanPredictionModel;  // ONNX 모델
    Worker worker;
    Tensor<float> tensor;
    private const int windowSize = 20;  // LSTM 입력 시퀀스 길이 (10 프레임)
    private const int targetLength = 197; // 학습에 사용한 보간 데이터 길이
    private const int numFeatures = 7;
    //학습에 사용된 표준화의 평균과 표준편차
    //private readonly float [ ] means = new float [ numFeatures ] { 0.74420376f , 0.43196177f , 0.45585604f , 0.65389165f , 4.26060081f , -30.2017924f , 12.81186023f };
    //private readonly float [ ] stdDevs = new float [ numFeatures ] { 0.44232754f , 0.28435855f , 0.23588109f , 0.27142065f , 16.65600266f , 30.62581823f , 26.16802147f };
    private readonly float [ ] means = new float [ numFeatures ] { 0.85383275f , 0.52744603f , 0.40626747f , 0.56567398f , 4.9705906f , -26.91390121f , 4.00129031f };
    private readonly float [ ] stdDevs = new float [ numFeatures ] { 0.50510986f, 0.27664916f , 0.24722899f , 0.34628094f , 15.75709249f , 28.38287609f , 21.32287882f };

    void Start ( )
    {
        // Sentis ONNX 모델 로드
        var runtimeModel = ModelLoader. Load ( supermanPredictionModel );
    ...
    void ProcessPredictions ( List<float> predictions )
    {
        ...
        else
        {
            ...
            resultText. text = "슈퍼맨 되기 동작 성공입니다.";
            averagePredictionText. text = ( $"Average Prediction: {averagePrediction:F2}" );
        }
    }
